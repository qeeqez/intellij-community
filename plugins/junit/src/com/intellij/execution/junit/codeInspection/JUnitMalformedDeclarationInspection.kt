...
private class JUnitMalformedSignatureVisitor(
  ...
  private fun checkSourceProvider(sourceProvider: UMethod, containingClass: PsiClass?, attributeValue: PsiElement, methodSourceAnnotation: PsiAnnotation, literal: PsiLiteralExpression?, method: UMethod) {
    ...
    val isNested = sourceProvider.javaPsi.containingClass?.containingClass != null
    val methodClass = sourceProvider.javaPsi.containingClass
    var fullClassName = ClassUtil.getJVMClassName(methodClass!!) + "#" + sourceProvider.name
    ...
    if (!sourceProvider.isStatic &&
      ...
      val message = JUnitBundle.message(
        "jvm.inspections.junit.malformed.param.method.source.static.descriptor", providerName)
      holder.registerProblem(place, message, *quickFixes)
    }
      ...
      val actions = createChangeAnnotationAttributeActions(
        ...
        JUnitBundle.message("jvm.inspections.junit5.assertions.converter.familyName")
      )
      val quickFixes = IntentionWrapper.wrapToQuickFixes(actions, sourceProvider.javaPsi.containingFile).toTypedArray()
      val message = JUnitBundle.message(
        "jvm.inspections.junit.malformed.param.method.source.external.class.descriptor", providerName)
      holder.registerProblem(place, message, *quickFixes)
    }
